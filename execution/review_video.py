...
def strip_ssml(text: str) -> str:
    """Strip SSML tags and convert phonetic aliases and breaks to plain text."""
    if not text:
        return ""
    clean = re.sub(r'<sub alias="([^"]+)">[^<]+</sub>', r'\1', text, flags=re.IGNORECASE)
...
def run_programmatic_checks(run_data: dict) -> list:
    ...
    full_script_raw = script_data.get("full_script", "")
    full_script = strip_ssml(full_script_raw)
    hook_raw = script_data.get("hook", "")
    hook = strip_ssml(hook_raw)
...
def review(run_id: str) -> dict:
    ...
    summary_str = json.dumps(run_data.get('summary', {}), indent=2)
    import re
