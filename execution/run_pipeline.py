...
def _run_weekly_autotune(channel: str, log_fn):
    ...
    try:
        ...
        # Step 0: fetch_analytics.py → updates experiments table with YouTube metrics
        r0 = _sp.run(
            [sys.executable, os.path.join(_root, "execution", "fetch_analytics.py")],
            capture_output=True, text=True, timeout=120, env=env
        )
        if r0.returncode != 0:
            log_fn(f"[BgIntel] fetch_analytics note: {r0.stderr[:200]}")

        # Step 1: analyze_performance.py → writes performance_findings_{channel}.json
        r1 = _sp.run(
            [sys.executable, os.path.join(_root, "execution", "analyze_performance.py")],
            capture_output=True, text=True, timeout=120, env=env
        )
        if r1.returncode != 0:
        ...
        r2 = _sp.run(
            [sys.executable, os.path.join(_root, "execution", "auto_tune.py")],
    ...
    try:
        ...
        while attempt <= MAX_ATTEMPTS:
            ...
            if not legal_check["is_safe"]:
                ...
                if story_swaps < 1:
                    log(f"Legal check flagged '{story['title']}'. Attempting alternative story swap...", run_id)
                    try:
                        ...
                        if _alt_story and _alt_story['title'] != story['title']:
                            log(f"Alternative story found for legal swap: {_alt_story['title']}", run_id)
                            story = _alt_story
                            attempt = 1
                            story_swaps += 1
                            correction_feedback = None
                        ...
                        log(f"Alternative story fetch failed during legal swap: {_se}", run_id)
            ...
            if not qc_check["approved"]:
                ...
                if attempt < MAX_ATTEMPTS:
                    ...
                    # Instead of raising a fatal ContentFailure that completely shuts down the channel's daily upload,
                    # log QC leniency warning, send low-quality notification, and proceed with forced upload
                    log(f"⚠️ [QC Leniency Bypass] Max attempts ({MAX_ATTEMPTS}) reached. Proceeding with best candidate script to preserve upload schedule: {qc_check['feedback']}", run_id)
                    try:
                        send_message(
                            f"⚠️ *Low Quality Upload (QC Bypass)* — {_channel_display}\n"
                            f"Run: {run_id}\n"
                            f"Story: {story['title']}\n"
                            f"QC Feedback: {qc_check['feedback']}\n"
                            f"Proceeding with forced upload to protect channel schedule."
                        )
                    except Exception:
                        pass
            ...
            # STEP 5.1: Thumbnail (from video clips for DemoChannel, AI for others)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--channel", type=str, default=None, help="Channel profile name (e.g. demo_channel)")
    parser.add_argument("--story", type=str, default=None, help="Custom story title or topic override")
    ...
    if args.channel:
        os.environ["ACTIVE_CHANNEL"] = args.channel
    if args.story:
        os.environ["STORY_OVERRIDE"] = args.story
