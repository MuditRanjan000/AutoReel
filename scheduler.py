...
# Track posts per channel: {"demo_channel": {"date": "2026-05-16", "count": 1}}
...
def run_daily_analytics():
    """Daily sync of YouTube Analytics metrics across all active channels."""
    active_channels = get_active_channels()
    if not active_channels:
        return
    for channel in active_channels:
        log(f"[CEO] Syncing daily analytics for: {channel.upper()}")
        env = os.environ.copy()
        env["ACTIVE_CHANNEL"] = channel
        try:
            subprocess.run([sys.executable, "execution/fetch_analytics.py"], env=env, timeout=180)
        except Exception as e:
            log(f"[CEO] Daily analytics sync error for {channel}: {e}")


# Schedule daily analytics sync at 00:30 IST
schedule.every().day.at("00:30").do(run_daily_analytics)
print("[CEO] Scheduled daily YouTube Analytics sync at 00:30 IST")
...
if __name__ == "__main__":
    ...
    try:
        from core.env_validator import validate_all
        validate_all()
    except ImportError:
        pass
