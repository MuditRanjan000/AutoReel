...
def init_db():
    ...
    try:
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_exp_video_id    ON experiments(video_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_exp_uploaded_at ON experiments(uploaded_at)")
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_seen_channel_seen_at ON seen_stories(channel, seen_at)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_seen_channel_url     ON seen_stories(channel, story_url)")
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_ab_active_created ON ab_tests(active, created_at)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_ab_video          ON ab_tests(video_id)")
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_replied_channel ON replied_comments(channel)")
...
def is_story_semantically_similar(channel: str, title: str, lookback_days: int = 3) -> bool:
    ...
    try:
        ...
        for (existing_title,) in rows:
            ...
            if ratio >= 0.6:
                log_msg = f"[DB] Semantic duplicate (overlap={ratio:.0%}): '{title[:50]}' ~ '{existing_title[:50]}'"
                try:
                    print(log_msg)
