...
FALLBACK_QUERIES_BY_NICHE = {
    # SurgeCulture — culture/dating niche: exclusively targeting attractive/sexy women from different locales
    "culture":  [
        {"source": "pexels", "query": "hot japanese woman tokyo night", "pexels_query": "hot japanese woman tokyo"},
        {"source": "youtube", "query": "sexy latina girl dancing salsa medellin", "pexels_query": "latina girl dancing salsa"},
        {"source": "pexels", "query": "beautiful blonde woman scandinavian", "pexels_query": "beautiful blonde woman"},
        {"source": "youtube", "query": "gorgeous brazilian girl rio beach", "pexels_query": "brazilian girl rio beach"},
        {"source": "pexels", "query": "attractive italian woman rome cafe", "pexels_query": "attractive italian woman rome"},
    ],
    # CrimeSurge — true crime, forensics, cctv, surveillance
    "crime": [
    ...
    # CricketSurge — cricket matches, stadiums, players
    "cricket": [
    ...
    # StoicSurge — statues, Roman/Greek ruins, rain, meditation
    "stoic": [
...
class VideoClipper:
    ...
    def _get_clip_queries(self, script_data: dict, recipe: dict = None) -> list[dict]:
        ...
        for scene in scenes:
            ...
            for v in visuals:
                ...
                # Read banned sources and niche from dynamic JSON config with safe fallbacks
                raw_cfg = self._ctx.raw_config if (self._ctx is not None and hasattr(self._ctx, "raw_config")) else {}
                banned_sources = raw_cfg.get("visuals", {}).get("banned_sources", [])
                niche_str = str(raw_cfg.get("NICHE", self._niche or "")).lower()
                bgm_allowed = raw_cfg.get("bgm_allowed_moods", [])
                ...
                if "pexels" in banned_sources:
                    if any(w in query.lower() for w in ["mugshot", "evidence", "photo", "news article", "article", "newspaper"]):
                        ...
                        source = "youtube"
                elif self._is_person_query(query) and any(w in niche_str for w in ["sports", "cricket"]):
                    source = "youtube"
                elif "cinematic" in bgm_allowed:
                    source = "pexels"
    ...
    def _search_youtube(self, query: str, out_path: str, timeout: int = 90, exclude_ids: set = None, visual_intent: str = None) -> str | None:
        ...
        try:
            ...
            if "sign in" in err_lower or "bot" in err_lower or "cookie" in err_lower or "n challenge" in err_lower or "js engine" in err_lower or "deno" in err_lower:
                if not getattr(self, "_cookie_alert_sent", False):
                    try:
                        from core.telegram_bot import send_message
                        if "n challenge" in err_lower or "deno" in err_lower or "js engine" in err_lower:
                            send_message("⚠️ *YouTube JS/Bot Protection Triggered!*\nyt-dlp failed to download footage (n challenge). JS Engine or Deno might be missing/misconfigured.\n\nError snippet: `" + stderr_text[-200:].replace('`', "'").strip() + "`")
                        else:
