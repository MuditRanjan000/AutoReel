        ...
        for url in self._rss_feeds:
            try:
                ...
                for entry in feed.entries[:10]:
                    ...
                    meta_keywords = [
                        "megathread", "episode", "discussion", "podcast", "update",
                        "read me first", "welcome!", "welcome to", "rules", "guidelines",
                        "weekly thread", "daily thread", "faq", "wiki"
                    ]
                    if any(bad_word in lower_title for bad_word in meta_keywords):
                        continue
...
"""
    ...
    def _generate_culture_topic(self, video_format: str) -> dict:
        """
        Unified topic generator for all DemoChannel video formats.
...
- "What Colombian Women Secretly Find Attractive" → 4.3M views  
- "Why You'll Never Understand Latina Women" → 4.3M views
- "Philippines Nightlife Survival Guide for Men" → 4M views
- "Why Eastern European Women Will Break You" → 3.7M views
...
"""
        try:
            ...
            FALLBACKS = {
                ...
                "C_culture_shock_place": f"What {country} Women Secretly Find Attractive",
                "D_expat_perspective":   f"3 Years Dating in {country} Changed Me",
                "E_meet_a_girl":         f"Culture Shocks When You Meet a {country} Girl",
                "F_survival_guide":      f"{country} Nightlife Survival Guide for Single Men",
                "G_will_shock_break":    f"Why Dating {country} Women Will Break You",
            }
