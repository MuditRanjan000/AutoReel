...
Usage:
    ...
    ctx = ChannelContext("demo_channel")     # direct instantiation for testing
"""
...
class ChannelContext:
    ...
    def __init__(self, channel_name: str, channels_dir: str = None):
        ...
        Args:
            channel_name: The channel slug, e.g. "demo_channel"
        ...
        self.raw_config = cfg
        self.config = cfg  # Alias for backward/forward compatibility

        # ── Channel identity ──────────────────────────────────────────────────
        self.display_name    = cfg.get("CHANNEL_NAME",        "DemoChannel")
        ...
        # Per-channel LLM skill override — e.g. DemoChannel uses 'culture-script-writer'
        ...
        pitch_raw = cfg.get("voice_pitch", 0.0)
        if isinstance(pitch_raw, (int, float)):
            self.voice_pitch = float(pitch_raw)
        else:
            match = re.search(r'([+-]?\d+(?:\.\d+)?)', str(pitch_raw))
            self.voice_pitch = float(match.group(1)) if match else 0.0
        ...
        self.show_watermark = bool(cfg.get("SHOW_WATERMARK", cfg.get("show_watermark", False)))
    ...
    def get(self, key, default=None):
        """Safe dictionary-like getter for raw channel config values."""
...
if __name__ == "__main__":
    ...
    print(ctx.summary())
    print()
    ctx2 = ChannelContext("demo_channel")
    print(ctx2.summary())
