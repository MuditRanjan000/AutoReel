...
def clean_text_spelling(text: str, is_cricket: bool = False) -> str:
    """Sanitize spelling, colloquial artifacts, and quotes consistently."""
...
Story summary  : {story.get('summary', '')[:2000]}
...
"""
    ...
    def _extract_scenes(self, script_data: dict, skill_context: str, min_scenes: int) -> dict:
        prompt = f"""
You are a professional video editor and director segmenting a completed YouTube Shorts narration script into distinct visual scenes for B-roll footage.

CRITICAL INSTRUCTION:
DO NOT REWRITE THE SCRIPT. Preserve the spoken text EXACTLY as written.
No rewriting, no summarization, no paraphrasing. Only segmentation into at least {min_scenes} scenes.
Break the narration into punchy chunks (0.5s to 4s) with literal physical subjects for stock footage search queries.

Here is the completed script to segment:
...
"""
    ...
    def generate(self, story: dict, recipe: dict = None, correction_feedback: str = None, ctx=None) -> dict:
        ...
        clean_fn = lambda txt: clean_text_spelling(txt, is_cricket=_is_cricket)

        for key in ["hook", "body", "cta", "full_script", "title", "description", "thumbnail_text"]:
            if key in script_data:
                script_data[key] = clean_fn(script_data[key])
        if "search_queries" in script_data:
            script_data["search_queries"] = [clean_fn(q) for q in script_data["search_queries"]]
        if "scenes" in script_data and isinstance(script_data["scenes"], list):
            for s in script_data["scenes"]:
                if isinstance(s, dict):
                    if "narration" in s:
                        s["narration"] = clean_fn(s["narration"])
                    if "query" in s:
                        s["query"] = clean_fn(s["query"])
                    if "tier1_query" in s:
                        s["tier1_query"] = clean_fn(s["tier1_query"])
                    if "visuals" in s and isinstance(s["visuals"], list):
                        for v in s["visuals"]:
                            if isinstance(v, dict):
                                if "query" in v:
                                    v["query"] = clean_fn(v["query"])
                                if "tier1_query" in v:
                                    v["tier1_query"] = clean_fn(v["tier1_query"])
