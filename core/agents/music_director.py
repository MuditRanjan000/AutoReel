...
"""
    ...
    def score_video(self, script: str, run_id: str, recipe: dict = None) -> str:
        ...
        if not os.path.exists(bgm_path):
            ...
            # Strictly filter for valid non-empty MP3 files (> 1KB)
            mood_files = [f for f in glob.glob(os.path.join(self.config_dir, f"{mood}*.mp3")) if os.path.isfile(f) and os.path.getsize(f) > 1024]
            
            if not mood_files:
                if mood == "cheerful":
                    mood_files = [f for f in glob.glob(os.path.join(self.config_dir, "upbeat*.mp3")) if os.path.isfile(f) and os.path.getsize(f) > 1024]
                elif mood == "dark":
                    mood_files = [f for f in glob.glob(os.path.join(self.config_dir, "suspense*.mp3")) if os.path.isfile(f) and os.path.getsize(f) > 1024]
            ...
            if mood_files:
                ...
                # Absolute safety fallback: try to find any valid .mp3 in bgm_defaults
                if os.path.exists(self.config_dir):
                    default_tracks = [
                        os.path.join(self.config_dir, f)
                        for f in os.listdir(self.config_dir)
                        if f.endswith(".mp3") and os.path.getsize(os.path.join(self.config_dir, f)) > 1024
                    ]
                    if default_tracks:
                        fallback_track = random.choice(default_tracks)
