...
class ThumbnailDesigner:
    ...
    def _create_prompt(self, title: str, script: str) -> str:
        ...
        """
        custom_inst = self.ctx.get("THUMBNAIL_PROMPT_INSTRUCTION") if self.ctx else None
        if custom_inst:
            prompt += f"""
        CRITICAL DESIGN INSTRUCTION: {custom_inst}
        """
        prompt += """
...
"""
    ...
    def generate(self, title: str, script: str, video_id: str,
        ...
        For DemoChannel: always extracts a real frame from the video clips.
        Scans multiple timestamps across multiple clips to find the best frame.
        Never uses Pollinations.ai for DemoChannel — authenticity is the brand.
        """
        output_path = os.path.join(self.thumbnail_dir, f"{video_id}_thumbnail.jpg")

        # ── DemoChannel / DemoChannel: Extract from real video clips ──────────
        is_culture = self.channel_slug and "culture" in self.channel_slug.lower()
        is_cricket = self.channel_slug and "cricket" in self.channel_slug.lower()
        
        if is_culture or is_cricket:
            if is_culture:
                print("[ThumbnailDesigner] DemoChannel: Scanning clips for best thumbnail frame...")
            else:
                print("[ThumbnailDesigner] DemoChannel: Scanning clips for best thumbnail frame...")
            ...
            if not best_frame:
                # PIL gradient fallback — no Pollinations for DemoChannel/DemoChannel fallback
                self._create_fallback_image(output_path, thumb_color)
            
            # For DemoChannel, return clean image directly (no text)
            if is_culture:
                return output_path, {"thumbnail_style": "Clean (No Text)"}
                
            # For DemoChannel, stamp text overlay on the extracted frame and return
        ...
        # ── Non-DemoChannel: AI generation path ──────────────────────────────
