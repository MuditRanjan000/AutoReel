...
import os
import json
from config.settings import TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, YOUTUBE_CHROMIUM_PROFILE_PATH
...
def _handle_command(text: str):
    ...
    if cmd == "/help":
        ...
        if valid:
            profile_path = YOUTUBE_CHROMIUM_PROFILE_PATH or "configured Chromium profile"
            send_message("✅ *Cookies are managed natively by the server and are valid.*\n"
                         f"The system uses the Chromium profile at `{profile_path}`.\n\n"
                         "Test Output:\n`" + msg + "`")
        else:
