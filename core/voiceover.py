...
def _extract_plain_text_with_phonetics(text: str) -> str:
    """
    Converts SSML text to clean plain text for non-SSML TTS engines (ElevenLabs, Edge TTS, Journey).
    Extracts phonetic pronunciations from <sub alias="..."> tags and converts breaks to pauses.
    """
    if not text:
        return ""
    clean = text.replace("&lt;", "<").replace("&gt;", ">").replace("&amp;", "&")
    # Extract phonetic aliases e.g. <sub alias="Hardik Pandya">Hardik Pandeya</sub> -> Hardik Pandya
    clean = re.sub(r'<sub alias="([^"]+)">[^<]+</sub>', r'\1', clean, flags=re.IGNORECASE)
    # Convert pauses to ellipses
    clean = re.sub(r'<break[^>]+>', '... ', clean, flags=re.IGNORECASE)
...
class VoiceoverGenerator:
    ...
    def _call_edge_tts(self, text: str, voice: str, rate: str, raw_wav: str):
        ...
        Args:
            text:    Plain text or SSML (phonetic aliases will be extracted).
        ...
        # Extract phonetic aliases and strip XML tags for Edge TTS
        plain_text = _extract_plain_text_with_phonetics(text)
        if not plain_text:
    ...
    def generate(self, script, filename: str, recipe: dict = None, ctx=None):
        ...
        #   ExampleChannel: "-5%"   (Edge TTS / percentage format)
        #   DemoChannel: "1.0"   (GCP float format, legacy)
        #   ElevenLabs voices: rate is ignored (baked into the voice model)
        # Normalize to a float speed_ratio (1.0 = normal) for GCP,
        # and to "+N%" percentage string for Edge TTS.
        speed_ratio = 1.0
        if "Journey" in self.voice_name:
            ...
            if is_ssml:
                print(f"[Voiceover] Journey models do not support SSML. Converting to plain text with phonetics...")
                script_clean = _extract_plain_text_with_phonetics(script_clean)
                is_ssml = False
        else:
            ...
            if rate_match:
                ...
                try:
                    ...
                    if 0.1 <= float_rate <= 4.0:
                        ...
                        # Normalize to percentage format for Edge TTS display
        ...
        if is_elevenlabs:
            ...
            el_text = _extract_plain_text_with_phonetics(script_clean)
            try:
                ...
                if _GCP_AVAILABLE:
                    print("[Voiceover] Falling back to Google Cloud TTS...")
                    is_gcp = True
                    self.voice_name = {
                        "JBFqnCBsd6RMkjVDRZzb": "en-GB-Journey-D", # DemoChannel (British male)
                        "XrExE9yKIg1WjnnlVkGX": "en-US-Journey-F", # DemoChannel (Female)
                        "nPczCjzI2devNBz1zQrb": "en-US-Journey-D", # ExampleChannel (Deep male)
                        "onwK4e9ZLuTAKqWW03F9": "en-US-Journey-D"  # DemoChannel (Deep male)
                    }.get(self.voice_name, "en-US-Journey-D")      # Catch-all default
                else:
