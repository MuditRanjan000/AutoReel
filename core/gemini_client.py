...
# Model cascade for Groq — ordered by quality/throughput to fallback
_GROQ_MODEL_CASCADE = [
    GROQ_MODEL,
    "openai/gpt-oss-120b",
    "groq/compound-mini",
...
def _is_groq_quota_error(e: Exception) -> bool:
    ...
    return any(term in err.lower() for term in [
        "429", "rate_limit", "limit reached", "tokens per day",
        "organization_restricted", "404", "model_not_found",
        "decommissioned", "does not exist", "400", "503", "unavailable",
        "413", "too large", "request_too_large"
    ])
...
def _call_openrouter(prompt: str, response_format: dict = None, temperature: float = 0.85) -> str:
    ...
    while i < len(models_to_try):
            ...
            for offset in range(num_keys):
                ...
                try:
                    ...
                    if "429" in err_str:
                        ...
                        last_error = e
                        continue
                    print(f"[OpenRouter] {label} error on {model}: {err_str} — trying next...")
                    last_error = e
                    continue
            
            if model_failed_fatal:
                break
                
            # If we reach here and haven't returned, all keys failed this attempt
            if _global_attempt < MAX_GLOBAL_RETRIES - 1:
        ...
        # If we reached the end of curated models and haven't dynamically fetched free models yet, fetch them
