...
class VideoAssembler:
    ...
    def merge_audio_video(self, overlays: list[str],
        ...
        # Standard 30.0 fps across all clips to guarantee exact Whisper subtitle sync
        fps_val = 30
    ...
    def add_text_overlay(self, video_path: str, title: str, output_name: str) -> str:
        ...
        if font_path:
            ...
            safe_channel = (self.channel_name or "").replace("'", "\\'").replace(":", "\\:")
            
            drawtext_filters = []
            show_watermark = getattr(self.ctx, "show_watermark", True) if self.ctx else True
            if show_watermark and safe_channel and safe_channel.strip() and safe_channel.lower() not in ["none", "false"]:
